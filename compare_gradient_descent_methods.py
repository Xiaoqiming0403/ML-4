...
RESULT_PATH = "gradient_descent_comparison.csv"
...
def stratified_split(y, train_ratio=0.8, seed=SEED):
...
def train(X, y, batch_size, lr=LR, epochs=EPOCHS, seed=SEED):
    n, d = X.shape
    ...
    return W, update_count, loss(X, Y, W)
...
def evaluate_method(name, batch_size, X_train, y_train, X_val, y_val, X_test, y_test):
    start = time.perf_counter()
    W, update_count, final_loss = train(X_train, y_train, batch_size=batch_size)
    ...
    return {
        ...
        "train_accuracy": accuracy(predict(X_train, W), y_train),
        "validation_accuracy": accuracy(predict(X_val, W), y_val),
        "test_accuracy": accuracy(predict(X_test, W), y_test),
    }


def print_markdown_table(results):
    print("| Method | Batch size | Epochs | Updates | Time (s) | Train Acc | Val Acc | Test Acc | Final Loss |")
    print("|---|---:|---:|---:|---:|---:|---:|---:|---:|")
    for row in results:
        print(
            f"| {row['method']} "
...
X_train, y_train, X_val, y_val, X_test, test_ids, classes = prepare_data(train_df, test_df)
answer_labels = answer_df.sort_values(ID_COL)[TARGET].to_numpy()
test_order = np.argsort(test_ids)
X_test = X_test[test_order]
y_test = np.array([np.where(classes == label)[0][0] for label in answer_labels])

methods = [
    ("Full-batch GD", len(X_train)),
    ("Mini-batch GD", 256),
    ("SGD", 1),
]

results = [
    evaluate_method(name, batch_size, X_train, y_train, X_val, y_val, X_test, y_test)
    for name, batch_size in methods
]

pd.DataFrame(results).to_csv(RESULT_PATH, index=False)
print_markdown_table(results)
print(f"\nSaved comparison to {RESULT_PATH}")
