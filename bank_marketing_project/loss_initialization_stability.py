import time
from pathlib import Path

import numpy as np
import pandas as pd


DATA_PATH = Path("data/bank_marketing/bank-additional/bank-additional-full.csv")
OUTPUT_DIR = Path("outputs/custom_losses")
TARGET = "y"
BASE_SEED = 42


CONFIGS = [
    {"name": "logistic_bce", "display_name": "Logistic BCE", "lr": 0.08, "epochs": 60, "l2": 0.0001},
    {"name": "linear_probability", "display_name": "Linear probability loss", "lr": 0.2, "epochs": 60, "l2": 0.0001},
]


def sigmoid(z):
    z = np.clip(z, -500, 500)
    return 1 / (1 + np.exp(-z))


def sample_weights(y):
    neg = np.sum(y == 0)
    pos = np.sum(y == 1)
    return np.where(y == 1, neg / pos, 1.0)


def loss_and_grad_z(name, z, y):
    p = sigmoid(z)
    eps = 1e-15
    if name == "logistic_bce":
        loss = -(y * np.log(np.clip(p, eps, 1 - eps)) + (1 - y) * np.log(np.clip(1 - p, eps, 1 - eps)))
        grad_z = p - y
    elif name == "linear_probability":
        loss = y * (1 - p) + (1 - y) * p
        grad_z = (1 - 2 * y) * p * (1 - p)
    else:
        raise ValueError(name)
    return loss, grad_z


def stratified_split(y, train_ratio=0.6, val_ratio=0.2, seed=BASE_SEED):
    rng = np.random.default_rng(seed)
    train_idx, val_idx, test_idx = [], [], []
    for label in np.unique(y):
        idx = np.where(y == label)[0]
        idx = rng.permutation(idx)
        n_train = int(len(idx) * train_ratio)
        n_val = int(len(idx) * val_ratio)
        train_idx.extend(idx[:n_train])
        val_idx.extend(idx[n_train:n_train + n_val])
        test_idx.extend(idx[n_train + n_val:])
    return (
        rng.permutation(np.array(train_idx)),
        rng.permutation(np.array(val_idx)),
        rng.permutation(np.array(test_idx)),
    )


def prepare_data():
    df = pd.read_csv(DATA_PATH, sep=";")
    y = (df[TARGET] == "yes").astype(float).to_numpy()
    X = pd.get_dummies(df.drop(columns=[TARGET]), drop_first=False).to_numpy(dtype=float)
    train_idx, val_idx, test_idx = stratified_split(y)
    X_train, y_train = X[train_idx], y[train_idx]
    X_val, y_val = X[val_idx], y[val_idx]
    X_test, y_test = X[test_idx], y[test_idx]

    mean = X_train.mean(axis=0)
    std = X_train.std(axis=0)
    std[std == 0] = 1
    X_train = (X_train - mean) / std
    X_val = (X_val - mean) / std
    X_test = (X_test - mean) / std

    X_train = np.column_stack([X_train, np.ones(len(X_train))])
    X_val = np.column_stack([X_val, np.ones(len(X_val))])
    X_test = np.column_stack([X_test, np.ones(len(X_test))])
    return X_train, y_train, X_val, y_val, X_test, y_test


def train(X_train, y_train, config, init_seed, init_scale):
    rng = np.random.default_rng(init_seed)
    n, d = X_train.shape
    w = rng.normal(0, init_scale, size=d)
    batch_size = 256
    start = time.perf_counter()

    for _ in range(config["epochs"]):
        idx = rng.permutation(n)
        X_shuffled = X_train[idx]
        y_shuffled = y_train[idx]
        for i in range(0, n, batch_size):
            Xb = X_shuffled[i:i + batch_size]
            yb = y_shuffled[i:i + batch_size]
            weights = sample_weights(yb)
            _, grad_z = loss_and_grad_z(config["name"], Xb @ w, yb)
            grad_w = Xb.T @ (weights * grad_z) / np.sum(weights)
            grad_w[:-1] += config["l2"] * w[:-1] / np.sum(weights)
            grad_w = np.clip(grad_w, -50, 50)
            w -= config["lr"] * grad_w

    return w, time.perf_counter() - start


def roc_auc(y_true, y_score):
    order = np.argsort(y_score)
    ranks = np.empty(len(y_score), dtype=float)
    ranks[order] = np.arange(1, len(y_score) + 1)
    pos = y_true == 1
    n_pos = np.sum(pos)
    n_neg = len(y_true) - n_pos
    return float((np.sum(ranks[pos]) - n_pos * (n_pos + 1) / 2) / (n_pos * n_neg))


def metrics(y_true, y_prob, threshold):
    pred = (y_prob >= threshold).astype(int)
    tp = int(np.sum((y_true == 1) & (pred == 1)))
    tn = int(np.sum((y_true == 0) & (pred == 0)))
    fp = int(np.sum((y_true == 0) & (pred == 1)))
    fn = int(np.sum((y_true == 1) & (pred == 0)))
    precision = tp / (tp + fp) if tp + fp else 0.0
    recall = tp / (tp + fn) if tp + fn else 0.0
    f1 = 2 * precision * recall / (precision + recall) if precision + recall else 0.0
    return {
        "accuracy": (tp + tn) / len(y_true),
        "precision": precision,
        "recall": recall,
        "f1": f1,
        "auc": roc_auc(y_true, y_prob),
    }


def best_threshold(y_true, y_prob):
    best_t, best_f1 = 0.5, -1
    for threshold in np.linspace(0.05, 0.95, 91):
        f1 = metrics(y_true, y_prob, threshold)["f1"]
        if f1 > best_f1:
            best_t, best_f1 = float(threshold), f1
    return best_t


def summarize(rows):
    summary = []
    df = pd.DataFrame(rows)
    for (loss, scale), group in df.groupby(["display_name", "init_scale"]):
        summary.append({
            "display_name": loss,
            "init_scale": scale,
            "runs": len(group),
            "mean_f1": group["test_f1"].mean(),
            "std_f1": group["test_f1"].std(),
            "min_f1": group["test_f1"].min(),
            "max_f1": group["test_f1"].max(),
            "mean_auc": group["test_auc"].mean(),
            "std_auc": group["test_auc"].std(),
            "mean_accuracy": group["test_accuracy"].mean(),
        })
    return summary


def markdown_table(rows, columns):
    lines = [
        "| " + " | ".join(columns) + " |",
        "| " + " | ".join(["---"] * len(columns)) + " |",
    ]
    for row in rows:
        values = []
        for col in columns:
            value = row[col]
            if isinstance(value, float):
                value = f"{value:.6f}"
            values.append(str(value))
        lines.append("| " + " | ".join(values) + " |")
    return "\n".join(lines)


def run():
    OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
    X_train, y_train, X_val, y_val, X_test, y_test = prepare_data()
    rows = []
    init_scales = [0.0, 0.1, 1.0, 3.0]
    seeds = list(range(3))

    for config in CONFIGS:
        for init_scale in init_scales:
            for seed in seeds:
                w, seconds = train(X_train, y_train, config, BASE_SEED + seed, init_scale)
                val_prob = sigmoid(X_val @ w)
                test_prob = sigmoid(X_test @ w)
                threshold = best_threshold(y_val, val_prob)
                row = {
                    "loss": config["name"],
                    "display_name": config["display_name"],
                    "init_scale": init_scale,
                    "seed": seed,
                    "threshold": threshold,
                    "time_seconds": seconds,
                }
                for key, value in metrics(y_test, test_prob, threshold).items():
                    row[f"test_{key}"] = value
                rows.append(row)

    summary = summarize(rows)
    pd.DataFrame(rows).to_csv(OUTPUT_DIR / "initialization_stability_runs.csv", index=False)
    pd.DataFrame(summary).to_csv(OUTPUT_DIR / "initialization_stability_summary.csv", index=False)

    columns = ["display_name", "init_scale", "runs", "mean_f1", "std_f1", "min_f1", "max_f1", "mean_auc", "std_auc", "mean_accuracy"]
    md = [
        "# Initialization Stability: Logistic BCE vs Linear Probability Loss",
        "",
        markdown_table(summary, columns),
    ]
    (OUTPUT_DIR / "initialization_stability_summary.md").write_text("\n".join(md), encoding="utf-8")
    print("\n".join(md))


if __name__ == "__main__":
    run()
